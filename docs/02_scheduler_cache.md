...
以 600-token 输入、每轮可用预算 256、块容量 256 为例，假设没有命中缓存且资源充足：

| 轮次 | 输入位置 | 成功后 committed | 请求持有 KV 块数 | GDN 槽 | 是否采样 |
| --- | --- | ---: | ---: | --- | --- |
| 1 | `[0,256)` | 256 | 1 | 同一个槽 | 否 |
| 2 | `[256,512)` | 512 | 2 | 同一个槽 | 否 |
| 3 | `[512,600)` | 600 | 3 | 同一个槽 | 是，生成首个 token |
| 4 | 位置 600 的首个生成 token | 601 | 3 | 同一个槽 | 是，生成第二个 token |
...
例如块容量为 256，某轮从位置 0 一次执行到 300，虽然 KV 中已有一个完整块，但 GDN 此刻表示 300-token 历史，代码不会反推位置 256 的状态，因此不会自动发布 256-token 快照。若先执行到 256，再继续剩余 44 个，就有机会发布前缀。
