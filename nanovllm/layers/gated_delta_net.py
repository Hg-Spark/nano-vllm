    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if context.is_prefill:
            ...
            for seq_idx, slot_id in enumerate(state_slots):
                ...
                if self.state_pool.allocated and slot_id >= 0:
                    if expected_prefix == 0:
                        self.state_pool.clear(slot_id)
                    conv_state, recurrent_state = (
                        self.state_pool.slot(slot_id)
                    )
                else:
                    if expected_prefix != 0:
                        raise RuntimeError(
            ...
            for token_idx, slot_id in enumerate(state_slots):
                if slot_id < 0 or not self.state_pool.allocated:
                    raise RuntimeError(
                ...
                conv_state, recurrent_state = self.state_pool.slot(
                    slot_id
                )
                outputs.append(
                    self._forward_sequence(
